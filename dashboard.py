...
# --- CONFIGURAÇÃO DA IA ---
# Verifica se a chave existe nos Secrets e configura a biblioteca
...
if file:
    try:
        ...
        # --- SIDEBAR (Configurações de Valor) ---
        ...
        if 'df_final' in st.session_state:
            ...
            with tab4:
                ...
                if selecao_questoes:
                    ...
                    if not df_f.empty:
                        ...
                        fig_dist = px.bar(df_counts, x="Opção", y="Qtd", color="Questão", barmode="group", category_orders={"Opção": ['A', 'B', 'C', 'D', 'E']})
            ...
            with tab5:
                ...
                if st.button("Gerar Relatório Estratégico com IA"):
                    ...
                    prompt = f"Gere um diagnóstico pedagógico. Média: {media_val:.2f}. Questões críticas: {piores_lista}."

                    # Lógica Try/Except com Fallback para evitar erro 404 e SyntaxError
                    try:
                        ...
                        try:
                            # Se falhar (404), tenta o modelo pro estável
