...
if "GEMINI_API_KEY" in st.secrets and st.secrets["GEMINI_API_KEY"].strip():
    ...
    st.warning("Chave API do Gemini não configurada nos Secrets do Streamlit.")
...
if file:
    try:
        ...
        if "Gabarito" not in xls.sheet_names or "RespAluno" not in xls.sheet_names:
            st.error("⚠️ O arquivo precisa ter as abas 'Gabarito' e 'RespAluno'.")
            st.stop()
        ...
        if num_questoes == 0:
            st.error("Nenhuma coluna de questão foi encontrada na aba RespAluno. Use colunas numéricas como 1, 2, 3...")
            st.stop()
        ...
        if st.button("🚀 Calcular Notas e Gerar Dashboard"):
            ...
            st.success("Notas calculadas com sucesso!")
        ...
        if "df_final" in st.session_state:
            ...
            with tab1:
                ...
                turmas_disponiveis = ["Todas"] + sorted(df_final["Turma"].dropna().astype(str).unique().tolist())
                ...
                nomes_disponiveis = ["Todos os Alunos"] + sorted(df_temp["Nome"].dropna().astype(str).unique().tolist())
            ...
            with tab3:
                ...
                fig_q = px.bar(df_analise_q, x="Questão", y="% Acerto", color="Questão", text="% Acerto")
                fig_q.update_traces(texttemplate="%{text:.1f}%", textposition="outside")
                fig_q.update_layout(xaxis=dict(type="category"), yaxis=dict(range=[0, 115]))
            ...
            with tab4:
                ...
                questoes_disponiveis = sorted(
                    df_dist["Questão"].unique(),
                    key=lambda x: int(re.sub(r"\D", "", str(x))) if re.sub(r"\D", "", str(x)) else 0
                )
                ...
                selecao_questoes = st.pills(
                    ...
                    default=questoes_disponiveis[0:1]
                )
            ...
            with tab5:
                ...
                st.write("Análise gerada pelo Gemini baseada nos resultados da turma.")

                if not API_OK:
                    st.error("A chave da API do Gemini não está configurada corretamente.")
                elif st.button("Gerar Relatório Estratégico com IA"):
                    try:
                        ...
                        media_val = st.session_state["df_final"]["Nota Final"].mean()
                        prompt = (
                            f"Gere um diagnóstico pedagógico em português, com sugestões objetivas para o professor. "
                            f"Média da turma: {media_val:.2f}. "
                            f"Questões críticas: {piores_lista}. "
                            f"Faça uma análise breve, clara e acionável."
                        )
