...
if uploaded_file:
    try:
        ...
        with st.sidebar:
            ...
            if serie_col:
                serie_opcoes = ["Todas"] + sorted(df_semestre[serie_col].astype(str).str.strip().dropna().unique().tolist())
                serie_sel = st.selectbox("Série", serie_opcoes)

            if pp_col:
                pp_opcoes = ["Todos"] + sorted(df_semestre[pp_col].astype(str).str.strip().dropna().unique().tolist(), key=natural_key)
                pp_sel = st.selectbox("PP", pp_opcoes)

            if disciplina_col:
                disc_opcoes = ["Todas"] + sorted(df_semestre[disciplina_col].astype(str).str.strip().dropna().unique().tolist())
        ...
        m3.metric("Disciplinas", f"{df_total_filtrado[disciplina_col].nunique() if disciplina_col else 0}")
        ...
        with c2:
            ...
            if pp_col:
                ordem_pp = sorted(df_semestre[pp_col].astype(str).str.strip().unique().tolist(), key=natural_key)
